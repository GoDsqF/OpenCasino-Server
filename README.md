...
- **Система аутентификации** — текущая реализация: верификация Google ID-token через `google-api-client`. **В процессе** замены на полноценный Spring Security WebFlux + JWT с поддержкой нескольких OAuth-провайдеров; см. ветку `feature/auth-spring-security` и `CLAUDE.md`.
...
PostgreSQL-инстанс провижится **отдельно** — приложение не управляет миграциями. Сейчас в коде описана только сущность `Players` (`game/model/Players.kt`); соответствующая таблица должна существовать на стороне БД к моменту запуска сервера. Опорная DDL:
...
> Файл схемы (`schema.sql`) был удалён при миграции на R2DBC. В рамках ветки `feature/auth-spring-security` (phase 2) планируется ввести Flyway/Liquibase и хранить миграции под `src/main/resources/db/`.
...
> Полный каталог сообщений и схемы payload-ов — в `config/MessageTypes.kt` и пакете `network/pack/`. Этот раздел не покрывает их целиком и будет дополнен по мере стабилизации протокола (в первую очередь после фазы 5 auth-роадмапа, когда идентичность игрока перестанет приходить в `data`).
...
| Spring Security + JWT | **в работе**, ветка `feature/auth-spring-security` |
| Multi-provider OAuth (Google + Yandex + GitHub) | запланировано (phase 6) |
| Refresh tokens + revocation | запланировано (phase 7) |
| CORS / rate-limit / audit log | запланировано (phase 8) |

Подробности по фазам auth-работ — в `CLAUDE.md`. Задачи трекаются в GitLab `oss/opencasino-server` (issues !2–!9).