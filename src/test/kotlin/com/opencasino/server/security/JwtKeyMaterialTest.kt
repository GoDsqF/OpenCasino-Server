...
class JwtKeyMaterialTest {
    ...
    @Test
    fun `fromProperties reads PEM from privateKeyPath and publicKeyPath`(
        @TempDir tmp: Path,
    ) {
        val pair = KeyPairGenerator.getInstance("RSA").apply { initialize(2048) }.generateKeyPair()
    ...
    @Test
    fun `fromProperties prefers inline PEM over file path`(
        @TempDir tmp: Path,
    ) {
        val pair = KeyPairGenerator.getInstance("RSA").apply { initialize(2048) }.generateKeyPair()
        val privPem = toPem("PRIVATE KEY", pair.private.encoded)
    ...
    @Test
    fun `fromProperties fails clearly when keyPath points to missing file`(
        @TempDir tmp: Path,
    ) {
    ...
    @Test
    fun `fromProperties fails clearly when keyPath points to unreadable file`(
        @TempDir tmp: Path,
    ) {
